 ...
 *
 */
void adfSetNative(const struct AdfNativeFunctions* nativeFunctions) {
    if (!adfEnv.nativeFct) adfEnv.nativeFct = (struct AdfNativeFunctions*)malloc(sizeof(struct AdfNativeFunctions));
    if (!adfEnv.nativeFct) (*adfEnv.wFct)("adfInitDefaultEnv : malloc");
    else {
        if (!nativeFunctions) adfInitNativeFct(); else
            *(struct AdfNativeFunctions*)adfEnv.nativeFct = *nativeFunctions;
    }
}
