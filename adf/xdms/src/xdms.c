...
int main(int argc, char **argv){
	...
	int ext;
	char iname[FNAME_MAXC+1], oname[FNAME_MAXC+1], *inm, *onm, *p, *q, *destdir=NULL;
	char tname[FNAME_MAXC];
	char cmdstr[2 * FNAME_MAXC + 20];
		...
		if ((cmd == CMD_UNPKGZ) || (cmd == CMD_EXTRACT)) {
			...
			if (ret == NO_PROBLEM) {
				if (cmd == CMD_UNPKGZ) {
...
#ifdef UNDER_DOS
						/*  DOS sucks  */
						snprintf(cmdstr, sizeof cmdstr, "gzip -cfqn %s >%s",tname,onm);
#else
						snprintf(cmdstr, sizeof cmdstr, "gzip -cfqn \"%s\" >\"%s\"",tname,onm);
#endif
					else
#ifdef UNDER_DOS
						snprintf(cmdstr, sizeof cmdstr, "gzip -cfqn %s",tname);
#else
						snprintf(cmdstr, sizeof cmdstr, "gzip -cfqn \"%s\"",tname);
#endif
					...
					if ((onm) && (strlen(onm)>0))
#ifdef UNDER_DOS
						snprintf(cmdstr, sizeof cmdstr, "readdisk %s %s",tname,onm);
#else
						snprintf(cmdstr, sizeof cmdstr, "readdisk \"%s\" \"%s\"",tname,onm);
#endif
					else
#ifdef UNDER_DOS
						snprintf(cmdstr, sizeof cmdstr, "readdisk %s",tname);
#else
						snprintf(cmdstr, sizeof cmdstr, "readdisk \"%s\"",tname);
#endif
